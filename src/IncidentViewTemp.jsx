...
Je reconnais avoir été informé(e) de la collecte et de l'utilisation de mes données
dans le cadre de ma prise en charge par l'établissement.

J'accepte que ma photographie soit prise et intégrée au workflow interne lorsque cela
est nécessaire à la sécurisation de mon parcours, à l'identitovigilance ou à la gestion
d'une situation opérationnelle.

Je consens également à la conservation de la preuve de ma signature électronique,
du document de consentement associé, ainsi que des métadonnées de date et d'heure.
`.trim();
...
function buildIncidentReportPayload({ patient, incident, incidentUi }) {
  return {
    patient: patient
      ? {
          id: patient.id,
    ...
    incident: incident
      ? {
          ...
          incidentLog: safeArray(incident.incidentLog),
        }
      : null,
    incidentUi,
  };
...
function resizeImageToDataUrl(file, maxWidth = 1280, quality = 0.85) {
...
<div><strong>Statut :</strong> ${escapeHtml(safe(currentStatus?.label, "—"))}</div>
<div><strong>Temps écoulé :</strong> ${elapsedMinutes !== null ? `${elapsedMinutes} min` : "—"}</div>
<div><strong>Dernier lieu vu :</strong> ${escapeHtml(safe(incidentUi?.lastSeenLocation, "—"))}</div>
<div><strong>Dernière heure connue :</strong> ${escapeHtml(formatShortDateTime(incidentUi?.lastSeenAt))}</div>
</div>

<h2>Contexte</h2>
<div class="grid">
<div class="block"><div class="label">Contexte</div>${escapeHtml(safeArray(incidentUi?.contextTags).join(" • ") || "Non précisé")}</div>
<div class="block"><div class="label">Vulnérabilité</div>${escapeHtml(safeArray(incidentUi?.vulnerabilityTags).join(" • ") || "Non précisée")}</div>
<div class="block"><div class="label">Danger pour lui-même</div>${escapeHtml(safeArray(incidentUi?.dangerSelfTags).join(" • ") || "Non précisé")}</div>
<div class="block"><div class="label">Danger pour autrui</div>${escapeHtml(safeArray(incidentUi?.dangerOthersTags).join(" • ") || "Non précisé")}</div>
<div class="block"><div class="label">Danger contextuel</div>${escapeHtml(safeArray(incidentUi?.dangerContextTags).join(" • ") || "Non précisé")}</div>
<div class="block"><div class="label">Description utile</div>${escapeHtml(description || "Non précisée")}</div>
</div>

<h2>Recherches service</h2>
<div class="block">${escapeHtml(safeArray(incidentUi?.internalZones).join(" • ") || "Aucune zone tracée")}</div>

<h2>PC sécurité</h2>
<div class="block">${escapeHtml(buildSecurityBrief(incidentUi) || "Aucun compte rendu sécurité")}</div>

<h2>Décision direction</h2>
<div class="block">
${escapeHtml(safeArray(incidentUi?.directionDecisions).join(" • ") || "Aucune décision direction tracée")}
${incidentUi?.directionNotes ? `<br/><br/><strong>Notes :</strong> ${escapeHtml(incidentUi.directionNotes)}` : ""}
</div>
...
<strong>${escapeHtml(formatShortDateTime(evt.at))}</strong> — ${escapeHtml(safe(evt.label))} (${escapeHtml(safe(evt.by))})
</div>`
        )
        .join("")
...
<div class="block">
  ${
    incidentUi?.patientPhotoDataUrl
      ? `<div><img src="${incidentUi.patientPhotoDataUrl}" alt="Photo patient" style="max-width:220px;border-radius:12px;border:1px solid #e5e7eb;" /></div>
         <div style="margin-top:8px;"><strong>Prise le :</strong> ${escapeHtml(formatShortDateTime(incidentUi?.patientPhotoCapturedAt))}</div>`
...
<h2>Consentement</h2>
<div class="block">
  <div><strong>Version :</strong> ${escapeHtml(safe(incidentUi?.consentTextVersion, CONSENT_VERSION))}</div>
  <div><strong>Accepté :</strong> ${incidentUi?.consentAccepted ? "Oui" : "Non"}</div>
  <div><strong>Signataire :</strong> ${escapeHtml(safe(incidentUi?.consentSignerName, "Non renseigné"))}</div>
  <div><strong>Signé le :</strong> ${escapeHtml(formatShortDateTime(incidentUi?.consentSignedAt))}</div>
  ${
    incidentUi?.consentSignatureDataUrl
      ? `<div style="margin-top:10px;">
          <img src="${incidentUi.consentSignatureDataUrl}" alt="Signature" style="max-width:260px;border:1px solid #e5e7eb;border-radius:12px;background:#fff;" />
         </div>`
      : `<div style="margin-top:8px;">Aucune signature enregistrée</div>`
  }
</div>

<h2>Clôture</h2>
<div class="block">
<div><strong>Lieu de retrouvaille / issue :</strong> ${escapeHtml(safe(incidentUi?.foundLocation, "Non renseigné"))}</div>
<div><strong>Commentaire final :</strong> ${escapeHtml(safe(incidentUi?.finalComment, "Non renseigné"))}</div>
</div>
</body>
</html>
`;
...
function SignaturePad({ value, onChange, height = 180 }) {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      />
      <div>
        <button type="button" className="pv-btn ghost" onClick={clear}>
...
export default function IncidentView() {
  ...
  const incident = useMemo(() => {
    ...
    return (
      incidents.find(
        (i) => String(i.patientId) === String(patient.id) && String(i.status) !== "closed"
      ) || null
    );
  }, [incidents, patient]);
  ...
  const menuItems = [
    ...
    { id: "consentement", label: "Photo & consentement" },
  ...
  const handlePatientPhotoChange = async (event) => {
    ...
    try {
      ...
      setIncidentUi((prev) => ({
        ...
        patientPhotoCapturedAt: new Date().toISOString(),
      }));

      if (incident) {
        addIncidentAction(incident.id, {
          label: "Photo patient capturée",
          by: "Service",
          role: "service",
          at: new Date().toISOString(),
          status: "done",
  ...
  const validateConsent = () => {
    ...
    if (!incidentUi.consentAccepted) {
      window.alert("Le consentement doit être accepté.");
      return;
    }

    if (!incidentUi.consentSignerName?.trim()) {
      window.alert("Le nom du signataire est requis.");
      return;
    }

    if (!incidentUi.consentSignatureDataUrl) {
      window.alert("La signature électronique est requise.");
      return;
    }
    ...
    addIncidentAction(incident.id, {
      label: "Consentement signé électroniquement",
      by: incidentUi.consentSignerName,
      role: "service",
      at: signedAt,
      status: "done",
      context: {
        consentVersion: incidentUi.consentTextVersion || CONSENT_VERSION,
        photoCaptured: !!incidentUi.patientPhotoDataUrl,
      },
    });
  };
...
const closeIncident = () => {
  ...
  const closedRecord = {
    ...
    summary: {
      ...
      patientPhotoDataUrl: incidentUi.patientPhotoDataUrl || "",
      patientPhotoCapturedAt: incidentUi.patientPhotoCapturedAt || "",
      consent: {
        version: incidentUi.consentTextVersion || CONSENT_VERSION,
        accepted: !!incidentUi.consentAccepted,
        signerName: incidentUi.consentSignerName || "",
        signedAt: incidentUi.consentSignedAt || "",
        signatureDataUrl: incidentUi.consentSignatureDataUrl || "",
      },
    },
  };

  writeJsonStorage(`incident_report_${incident.id}`, closedRecord);
  writeJsonStorage(`patient_last_incident_${patient.id}`, closedRecord);
  appendPatientIncidentHistory(patient.id, closedRecord);
          ...
          >
              ...
              {menuItems.map((item) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 16,
                    border:
                      activeSection === item.id ? "1px solid #bfd3ff" : "1px solid #edf2f7",
            ...
            <main className="incident-main" style={{ display: "grid", gap: 18 }}>
                ...
                <div style={{ display: "grid", gap: 16 }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <InfoBox
                        label="PC sécurité"
                        value={securityAlertDone ? "Alerté" : "Non alerté"}
                      />
                      <InfoBox
                        label="Direction"
                        value={directorAlertDone ? "Alertée" : "Non alertée"}
                      />
                    </div>
                  </SectionCard>
                  ...
                  <SectionCard
                    title="Photo & consentement"
                    subtitle="Éléments d'identification et preuve de consentement."
                  >
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                      <InfoBox
                        label="Photo patient"
                        value={incidentUi.patientPhotoDataUrl ? "Enregistrée" : "Non enregistrée"}
                      />
                      <InfoBox
                        label="Consentement"
                        value={incidentUi.consentAccepted ? "Accepté" : "Non accepté"}
                      />
                      <InfoBox
                        label="Signature électronique"
                        value={
                          incidentUi.consentSignedAt
                            ? formatShortDateTime(incidentUi.consentSignedAt)
                            : "Non signée"
                        }
                      />
                    </div>
                  ...
                  >
                    <div style={{ display: "grid", gap: 12 }}>
                      ...
                      {incidentUi.patientPhotoDataUrl ? (
                        ...
                        >
                          <img
                            src={incidentUi.patientPhotoDataUrl}
                            alt="Photo patient"
                            style={{
                              width: "100%",
                              borderRadius: 12,
                          ...
                          <div style={{ fontSize: 12, color: "#64748b" }}>
                            Capturée le {formatShortDateTime(incidentUi.patientPhotoCapturedAt)}
                          </div>
                        </div>
                      ) : (
                    ...
                    <div style={{ display: "grid", gap: 14 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <label style={{ display: "grid", gap: 6 }}>
                          ...
                          <input
                            value={incidentUi.consentSignerName}
                            onChange={(e) => setUiField("consentSignerName", e.target.value)}
                        ...
                        <label style={{ display: "grid", gap: 6 }}>
                          ...
                          <input
                            value={incidentUi.consentTextVersion}
                            readOnly
                            style={{
                              minHeight: 40,
                              borderRadius: 12,
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={incidentUi.consentAccepted}
                          onChange={(e) => setUiField("consentAccepted", e.target.checked)}
                        />
                        J’ai lu le formulaire et je donne mon consentement
                      </label>
                      ...
                      <div>
                        ...
                        <SignaturePad
                          value={incidentUi.consentSignatureDataUrl}
                          onChange={(value) => setUiField("consentSignatureDataUrl", value)}
                        />
                      </div>

                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                        <button type="button" className="pv-btn primary" onClick={validateConsent}>
                          Valider le consentement
                        </button>

                        {incidentUi.consentSignedAt ? (
                          <StatusChip color="green">
                            Signé le {formatShortDateTime(incidentUi.consentSignedAt)}
                          </StatusChip>
                        ) : (
                          <StatusChip color="amber">Consentement non validé</StatusChip>
                        )}
                      </div>
                    </div>
                  </SectionCard>
              ...
              {activeSection === "security" && (
                <div style={{ display: "grid", gap: 16 }}>
                  ...
                  <button
                    ...
                    onClick={validateSecurityChecks}
                  >
              ...
              {activeSection === "direction" && (
                <div style={{ display: "grid", gap: 16 }}>
                  <SectionCard title="Direction" subtitle="Décision et consignes tracées.">
                    <ChipToggleGroup
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    ...
                    <button
                      type="button"
                      className="pv-btn primary"
                      onClick={validateDirectionDecision}
                    >
